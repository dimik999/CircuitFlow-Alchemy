...
using CircuitFlowAlchemy.Game.FactoryAlchemy;
using UnityEngine;
#if ENABLE_INPUT_SYSTEM
using UnityEngine.InputSystem;
#endif

namespace CircuitFlowAlchemy.Game
{
    /// <summary>
    /// Emergency playable MVP for diploma demo:
    /// gather essences, craft mixes, complete first orders.
    /// </summary>
    public class DiplomaGame : MonoBehaviour
    {
        ...
        private static void AutoCreate()
        {
            ...
            if (FindFirstObjectByType<DiplomaGame>() != null)
            {
                return;
            }

            var go = new GameObject("DiplomaGame");
            DontDestroyOnLoad(go);
            go.AddComponent<DiplomaGame>();
        }
